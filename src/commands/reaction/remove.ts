...
export default class RemoveCommand extends Command {
			...
			args: [
				{
					...
					prompt: {
						...
						retry: 'Please provide a valid identifier for the reaction role you\'d like to delete. You can also delete the whole message to delete reaction roles on it.'
					}
				}
			]
		});
	...
	public async exec(msg: Message, { reaction }: { reaction: Reaction }): Promise<Message | Message[]> {
		...
		try {
			const chan = this.client.channels.get(reaction.channelID) as TextChannel;
			...
			await message.reactions.get(reaction.emoji)!.users.remove(this.client.user!.id)
		} catch (err) {
			this.client.logger.error(`[ERROR in REMOVE CMD]: ${err}.`);
		}

		this.client.settings!.set('reaction', { messageID: reaction.messageID }, {
			active: false
		});
